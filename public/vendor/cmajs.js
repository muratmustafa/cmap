  ...
  runtimes = (function () {
    ...
    runtimesInterface = {
      add: function (runtime) {
        runtimeLookup[runtime.name] = runtime;
      },
      remove: function (runtime) {
        delete runtimeLookup[runtime.name];
      },
...
(function () {
  ...
  template.pubSub.publish = function (args) {
    ...
    cmajs.runtimes.qtRuntime.mediator.publish({
      channel: args.channel,
      message: args.message,
      sender: {
        id: senderId
  ...
  template.pubSub.subscribe = function (args) {
    ...
    cmajs.runtimes.qtRuntime.mediator.subscribe({
      channel: args.channel,
      callback: args.callback
    });
    success = true;
  ...
  template.pubSub.unsubscribe = function (args) {
    ...
    cmajs.runtimes.qtRuntime.mediator.unsubscribe({
      channel: args.channel,
      callback: args.callback
    });
    success = true;
  ...
  cmajs.runtimes.qtRuntime = template;

}());

(function () {
  cmajs.runtimes.qtRuntime.mediator = (function () {
    var listeners = new cmajs.utils.Hash(),
      publicInterface = {
        ...
        publish: function (message) {
          ...
          try {
            // message.message kullan (message.payload değil - browser runtime'a bak)
            if (typeof message.message === "string") {
              message.message = JSON.parse(message.message);
            }
            
            // Qt bridge varsa mesajı gönder
            if (window.qtBridge && window.qtBridge.receiveMapMessage) {
              window.qtBridge.receiveMapMessage(message.channel, message.payload);
            } else {
              // Qt yoksa sessizce başarısız (tarayıcıda çalışıyor olabilir)
              success = {
                success: false,
                message: "qtBridge not available"
              };
            }
          } catch (err2) {
            success = {
        ...
        // Qt → JS mesajları için internal dispatch (QWebChannel signal'dan çağrılır)
        dispatchFromQt: function (msgChannel, payload) {
          var callbacks = listeners.getItem(msgChannel),
            len,
            i;
          ...
          if (callbacks !== null && callbacks !== undefined) {
            ...
            for (i = 0; i < len; i += 1) {
              if (callbacks[i] !== null) {
                try {
                  // Callback'i tetikle (sender, message, channel)
                  callbacks[i]({ id: 'qt-bridge' }, payload, msgChannel);
                } catch (err) {
                  console.error('[CMAPI Qt Runtime] Callback error:', err);
                }
              }
            }
          }
...
// Qt WebChannel bağlantısı - sadece Qt ortamında çalışır
(function () {
  // window.qt ve QWebChannel varsa Qt ortamındayız
  if (typeof window.qt !== 'undefined' && typeof QWebChannel !== 'undefined') {
    try {
      new QWebChannel(window.qt.webChannelTransport, function (channel) {
        // MainWindow.cpp'de registerObject("qtBridge", ...) ile kaydedildi
        window.qtBridge = channel.objects.qtBridge;
        
        console.log('[CMAPI Qt Runtime] QWebChannel connected');
        
        // Qt → JS mesajları için signal'ı dinle
        if (window.qtBridge && window.qtBridge.sendMapMessage) {
          window.qtBridge.sendMapMessage.connect(function (msgChannel, payload) {
            console.log('[CMAPI Qt Runtime] Received from Qt:', msgChannel, payload);
            
            // Payload'ı parse et (QJsonObject → JS object olarak gelir ama string de olabilir)
            ...
            // CMAPI subscriber'larını tetikle
            cmajs.runtimes.qtRuntime.mediator.dispatchFromQt(msgChannel, parsedPayload);
          });
        }
      });
    } catch (err) {
      console.error('[CMAPI Qt Runtime] QWebChannel connection failed:', err);
    }
  }
}());
...
/*global cmajs, tv4 */
  ...
  cmajs.init({
   runtimes: [cmajs.runtimes.browser, cmajs.runtimes.qtRuntime]
  })
